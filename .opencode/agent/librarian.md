...
### Write-routing для execution specs
- Перед созданием или записью spec определи целевой проект и прочитай его карточку
  `03-Projects/<project>.md`; поле `spec-home` — единственный путь назначения.
- Spec для чужого проекта записывается только в его `spec-home` через named
  project/build-субагента. `docs/specs/` этого Vault используется только для
  `project: vault` и не является staging-каталогом для чужих specs.
- После создания проверь физический путь: он должен находиться внутри
  заявленного `spec-home`; иначе остановись с `BLOCKED` и не оставляй копию в Vault.
- При `kind: task` активный spec находится в корне `spec-home`; после verifier
  PASS + commit/tag проектный агент переносит его в `spec-home/done/`.
