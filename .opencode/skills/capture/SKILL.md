...
- «покажи все абузы» / «capture Абуз --all»
...
## Специальный режим: мониторинг промо-провайдеров

Для темы **Абуз** применяется протокол [[02-Methods/promo-provider-protocol]]:
1. Извлечь посты через `/capture Абуз`
2. Для каждого поста с упоминанием API-провайдера:
   - Зафиксировать: домен, endpoint, условия промо, баланс, expiry
   - Провести PROBE (read-only): DNS/TLS, `/v1/models`, smoke-чат
   - Если `data: []` → `DEGRADED/BLOCKED`, не подключать
   - Если модели есть → создать карточку `01-Reference/provider-cards/<id>.md`
   - Подключить через CONNECT (TUI/M Code/dotfiles)
3. Карточки ведутся в `01-Reference/provider-cards/`, статусы обновляются через balance hook (T-144)
...
| `all` | Извлечь все непомеченные (без лимита) | false |
...
- **Абуз** — промо/реферальные провайдеры, бесплатные кредиты, акции
...
| 💸 | abuse | Промо/абуз провайдер (бесплатные кредиты, акции) |
...
- `02-Methods/promo-provider-protocol.md` — метод приёмки промо-провайдеров (для темы Абуз)
- `01-Reference/provider-cards/` — карточки провайдеров (результат обработки абузов)
