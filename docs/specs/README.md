...
Каждая spec несёт во фронтматтере `kind`:

- `kind: task` — одноразовая работа. После независимого verifier PASS +
  commit/tag **исполнивший агент переносит файл в `spec-home/done/`** (в своём
  репозитории). Отсутствие `kind` = `task`.
- `kind: contract` — долгоживущий канонический контракт (schema / протокол /
  роль). Лежит в корне `spec-home/`, **никогда** не переносится в `done/`.

`done/` — архив исполненных `task`-спек, чтобы активные `spec-home/*.md`
показывали только «в работе / предстоит». Перенос — lifecycle-шаг, **не evidence**:
факт выполнения доказывают git-история (commit/tag) и отчёт verifier, а не
наличие файла в `done/`. Перенос без независимого verifier PASS запрещён.
