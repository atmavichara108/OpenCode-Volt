...
## Текущий фокус
- **Ekstasis заведён 2026-09-24:** новый литературный проект Макса Рудры / Александра Майи, поток «В поисках Пустоты».
- **Proposal профиля писательской идентичности подготовлен 2026-09-24:** ожидает explicit user approval + `profile-governor`; канонический профиль не изменён.
- **Ekstasis (2026-09-25):** первое чтение «В поисках Нетленного» полностью валидировано автором; literary-workflow внедрён; инцидент runtime-прав исправлен. Следующий шаг — живой gate-smoke, затем полный проход картографа по трём частям; вопрос по стихам остаётся открытым.
...
- **JustDoWork DEGRADED:** 1 модель `claude-opus-4-8` в каталоге, но chat под Cloudflare captcha — нужна браузерная сессия, ключа мало.
...
- **Doom-loop решение (2026-09-17):** Rudra утвердил `ask` (не `deny`); зафиксировано в `librarian.md` и `opencode.json`.
- **Promo-provider protocol (2026-09-16):** метод [[02-Methods/promo-provider-protocol]] принят; карточки [[01-Reference/provider-cards/linaliapi]] (✅ ACTIVE) и [[01-Reference/provider-cards/justwoker]] (❌ BLOCKED/NO_MODELS) зафиксированы. Hook-spec `~/dotfiles/docs/specs/promo-provider-probe-balance-hook.md` ждёт dotfiles/sysop реализации (T-144).
- **Фокус: T-143 (P0)** — устранить мутацию живых tool-input в replay-budget: фиксы helpers (`structuredClone`, `prompt`/`content` keep, граница `i !== newestAssistantIndex`) уже лежат в `dotfiles/opencode-global/`, требуется запуск тестов, проверка в живом рантайме и верификатор.
- **Live-наблюдение (2026-09-19):** подтверждено активное срабатывание replay-budget на чтении истории сессии (`[mcode: replay budget — omitted N chars …]`).
...
2026-09-25 — **Ekstasis:** первое чтение валидировано автором, literary-workflow внедрён, runtime-права исправлены; следующий фокус — gate-smoke, затем картограф.

2026-09-24 — **Bootstrap Ekstasis:** заведены карточка проекта, индекс, T-150 и записи памяти. Следующий фокус — первое чтение, затем картография; registry ECO намеренно не изменён.
