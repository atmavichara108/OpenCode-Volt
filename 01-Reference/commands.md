...
| [`/agents`](file:///home/rudra/dotfiles/opencode-global/.config/opencode/command/agents.md) | `/home/rudra/dotfiles/opencode-global/.config/opencode/command/agents.md` → `~/.config/opencode/command/agents.md` | Открывает Pip-Boy на вкладке MODELS — переключение моделей агентов (deep-link `#models` через `pipboy-rofi models`) |
