...
Программный слой: прямой `edit` на hot-files переведён в `ask` в глобальных
конфигах (`~/.config/opencode/opencode.jsonc` и `~/.config/mcode/opencode.jsonc`):
...
"edit": {
  ...
  "**/TASKS.md": "ask",
  "**/00-INDEX.md": "ask",
  "**/active-context.md": "ask",
  "**/registry.json": "ask",
  "**/AGENTS.md": "ask"
}
```

Это заставляет агента идти через lease/bash-путь для общих файлов вместо
молчаливой перезаписи. Полный запрет мутattr в `main` требует
плагина-гейта (отдельный этап, по образцу `replay-budget.ts`).
