...
> Canonical source of truth для execution-решений по проекту dv-hub, в Vault.
> Локальный pointer — `.opencode/commands/spec.md` (рид-ронни на этот каталог).
> Правило One source ([[06-Specs/README]]): execution spec пишется **только здесь**;
> dv-hub-агент читает его до изменения кода/конфигурации.
...
- Канонические спеки и стратегия — здесь.
- Kanban-карточка экосистемы — `tools/ecosystem-map/registry.json` (ECO-022), ведётся
  Pip-Boy-сессией, не этим каталогом.