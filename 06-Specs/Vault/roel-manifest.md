...
- Decision Queue: `06-Specs/Vault/decision-queue/` — JSON-карточки (`YYYY-MM-DD-<slug>.json`) + `SCHEMA.md` + `runtime-events.jsonl` (append-only).
...
- Modules: `ecosystem`, `projects`, `upgrade`, `terminal`, `browser`,
  `acceptance`, `dependency`, `link`, `launcher`.
